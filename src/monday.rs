...
impl MondayClient {
    ...
    pub async fn search_tasks(&self, query: &str) -> Result<Vec<MondayTask>> {
        let graphql_query = if let Some(board_id) = &self.board_id {
    ...
    // Helper method to perform broader search including updates
    ...
    fn parse_task_item(&self, item: &Value) -> Option<MondayTask> {
        ...
        let column_values = if let Some(columns_array) = item["column_values"].as_array() {
            columns_array
                .iter()
