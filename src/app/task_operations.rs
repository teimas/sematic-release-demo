...
use crate::{
    app::App,
    services::{MondayClient, JiraClient},
    types::{MondayTask, JiraTask},
};
...
impl TaskOperations for App {
    ...
    async fn search_jira_tasks(&self, query: &str) -> Result<Vec<JiraTask>> {
        ...
        writeln!(debug_file, "DEBUG: search_jira_tasks called with query: '{}'", query).ok();
        ...
        match &result {
            Ok(tasks) => {
                ...
                for (i, task) in tasks.iter().enumerate().take(3) {
                    writeln!(debug_file, "DEBUG: JIRA Task {}: {} ({})", i, task.summary, task.key).ok();
                }
            }
            Err(e) => {
    ...
    fn update_task_selection(&mut self) {
        ...
        match self.config.get_task_system() {
            crate::types::TaskSystem::Monday => {
                ...
                let task_ids: Vec<String> = self.selected_monday_tasks.iter().map(|t| t.get_id().to_string()).collect();
            ...
            crate::types::TaskSystem::Jira => {
                ...
                let task_ids: Vec<String> = self.selected_jira_tasks.iter().map(|t| t.key.clone()).collect();
                self.commit_form.scope = if task_ids.is_empty() {
                    String::new()
                } else {
                    task_ids.join("|")
